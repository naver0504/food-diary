...
public interface ImageRepository extends JpaRepository<Image, Integer> {
  ...
  //  Optional<Image> findParentImageByImageIdAndUserId(@Param("imageId") int imageId, @Param("userId") int userId);
    ...
    Optional<Image> findByImageIdAndUserId(@Param("imageId") long imageId);

   // @Query("select i from Image i left join fetch i.diaryTags T where i.id = :id and i.parentImage.id is null and i.user.id = :userId")
